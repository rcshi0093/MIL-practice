...
class Generic_MIL_Dataset(Generic_WSI_Classification_Dataset):
	def __init__(self,
		...
		**kwargs):
		...
		self.use_h5 = False
		# 新增：初始化归一化参数（后续在训练集上计算）
		self.feat_mean = None  # 特征均值
		self.feat_std = None   # 特征标准差
	...
	def __getitem__(self, idx):
		...
		# 改为按位置访问（iloc）而不是按索引（loc）
		slide_id = str(self.slide_data.iloc[idx]['slide_id']).strip()  # 确保转为字符串
		...
		if features.size(0) == 0:
			# 方案1：抛出明确错误（推荐，便于定位空 bag）
			raise ValueError(f"Bag {slide_id}.pt 为空！实例数为 0，请检查该文件。")
			# 方案2：若想跳过空 bag（需配合循环，不推荐在 __getitem__ 中用）
			# return None  # 后续需在 DataLoader 中过滤 None

		# 正常返回特征和标签
		label = self.slide_data.iloc[idx]['label']
